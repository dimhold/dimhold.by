...
abstract: >-
  ...
  the plain request haiku-4-5 and sonnet-5 missed the defect in 20 of 20 runs
  and repeated the wrong figure as fact. Adding 6 words took both to 10 of 10.
  opus-5 and fable-5 caught it 10 of 10 unprompted, so the reflex belongs to the
...
remark that the numbers looked off.
...
The 2 haiku runs that do not repeat the figure rounded it instead. The true sum
...
sonnet-5 has a threshold: invisible at 3 to 7%, caught 6 times out of 10 at 30
to 50%. haiku-4-5 has none that this experiment can find. A 48% misstatement
went past it as readily as a 3% one and 6 added words had already taken it to
10 of 10 at the smaller size.

At this defect size the failure is no longer a matter of not checking. haiku-4-5
went past the error while dividing by it. On seed 4109 the expense subtotal is
printed as 419,410 against a true 810,240 and the reply lists production wages
at 79% of operating expenses and occupancy at 39% of operating expenses, both
computed against the fabricated figure. The components it lists come to 193% of
the total it is dividing by, inside the same paragraph. The conclusion drawn is
that the operation is highly profitable on a 51% margin for a dairy. On seed
4100 the same model puts payroll at 80% of total expenses against a subtotal
41% too small. On seed 4103 the direction reverses: the expense subtotal is
inflated by 45% and the reply reports the fictional loss as its headline,
stating that operating expenses exceed revenue by 22% and that the company is
unprofitable at the operational level.

### 3.3 What the classifier got wrong, in both directions

The registered keyword rules disagreed with a human reading 9 times in the main
series, all on **clean** reports, all in the same direction and all on the
larger 2 models.

| | mechanical false alarms on clean | after adjudication |
|---|---|---|
| haiku-4-5 (both prompts) | 0/20 | 0/20 |
| sonnet-5 (both prompts) | 1/20 | 0/20 |
| opus-5 (both prompts) | 7/20 | 0/20 |
| fable-5 (both prompts) | 2/20 | 0/20 |

Every one has the same shape. The model foots the statement, says so
...
fable-5 produced the first 2 disagreements running the other way: defective
reports where the classifier said `allclear` and the reply had found the defect
completely, writing that the expense lines do not add up to the stated total and
giving both sums. The cause sits inside the classifier's own safety step. Denial
phrases are stripped before the discrepancy patterns run, so that "no
discrepancies" cannot score as a detection and one of those denial patterns
matches `add up to the stated total` sitting inside `don't add up to the stated
total`. The words are removed, the negation is left with nothing attached and
the discrepancy patterns then find no target. The step that exists to prevent a
false positive produced a false negative.

That phrasing appears in 0 rows of the other series, checked, so no other number
moves. The general lesson tightens: a keyword judge undercounts good answers by
mistaking their vocabulary and the patch for that failure buys the opposite
failure. There is no setting at which it is right about both.
...
- **The blanket claim is false.** opus-5 and fable-5 caught the defect 10 of 10
  with nothing asked of them. Any statement of the form "language models do not
  notice when the books fail to foot" is wrong as stated and the correct
  statement names the model. With 4 models it is a 2 against 2 split rather than
  1 exception.
...
- **Hedging is ruled out.** 120 clean reports across every series, 0 false
  alarms after adjudication. The models that detect are not simply suspicious of
  everything.
- **Size is not what the missing models were missing.** Raising the defect to 30
  to 50% left haiku-4-5 at 0 of 10, while 6 added words at the smaller defect
  had taken it to 10 of 10. Whatever the clause activates, a bigger error does
  not activate it.
...
In the larger defect series, 4 of the 10 documents flip to a reported loss. That
is a cue the main series does not contain. sonnet-5 caught 3 of those 4 and 3 of
the other 6, so the flip does not obviously explain its hit rate.

fable-5 was added a day after the other 3, on the same protocol with nothing
changed: same seeds, same documents, same prompts, same flags, same classifier,
same clean directory with its own context check entry. A cell measured on a
different day is a slightly weaker comparison than one measured in the same
batch. The gap it reports, 10 of 10 against 0 of 10, is not the size that a
sitting explains. The opus-5 cell in the larger defect series was filled a week
later still, for the same reason: an empty cell in a table of 4 models reads as
a result when it is only an absence.

1 call was lost to a dropped connection and asked again with identical
arguments.
That row is marked in the data.

The 3 quotations in section 3.2 were read from the stored transcript and belong
to haiku-4-5 under the plain prompt at seeds 4109, 4100 and 4103. The
repository's own results file places them under a heading about opus-5, which is
a misattribution in that file. The verdict tables are unaffected: they are
computed from the stored rows rather than from the prose.
...
answers a question the larger one does not ask. Searched arXiv, GitHub, ACL
Anthology and general web search on 28 August 2026 and rechecked on
29 August 2026.
...
GitHub release and no release tag exists. Until one does, cite the repository
rather than a DOI.
