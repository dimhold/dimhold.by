...
subtitle: 'Two Anthropic models, one MCP tool broken five ways, 100 calls judged against the server log'
abstract: >-
  Language model agents are trusted to report when a tool they called did not
  work. This study separates two kinds of not working and finds that the
  distinction decides everything. A purpose built MCP server returned a token
  under five conditions: a permission error, an empty result, a truncated value,
  a substituted value and a correct control. Two Anthropic models were asked for
  the token across two prompt framings, 100 calls in total, and every reply was
  judged deterministically against the server's own log rather than against the
  ...
  in all 40. Two unrelated corruption shapes produced identical behaviour, which
...
An agent that calls a tool and gets nothing useful back has two jobs: notice,
and say so. The second one is what a user actually depends on, because a user
cannot see the tool call. Most of what is written about this treats it as one
behaviour with one rate.

It is not one behaviour. A tool can fail in a way that announces itself, and it
can fail in a way that does not. A permission error arrives labelled. A stale
cache, a partial read, a silently rate limited API and a truncated stream all
arrive looking exactly like success. This study measures the two cases
separately and finds that they are not close.
...
exposes one tool, `read_probe_file`, and it appends what it returned to a log
file on every call. **That log is the ground truth**, not the reply, and not
anyone's reading of the reply.

Five modes fall into two groups.
...
inside as a swapped one. The two modes are here to show what silent bad data
...
per call, so it exists in no training set and in no prompt. Two framings, because
...
built in file reader, the model never reaches the probe server, and the run
...
demanded 4 characters after the prefix, and pulled the word "Unable" out of
`TOKEN: Unable to read file` as if it were an asserted value. All 3 faults were
found by reading the replies the classifier had bucketed, and fixed by
...
value, and they produced identical behaviour: 20 of 20 relayed in each, no
hedge, no remark that the value looked short, nothing. That is what a single
mechanism predicts, and it is the reason to read the split as structural rather
than as an artefact of one injected fault.
...
Three cases, taken together with the earlier run:
...
The middle case is the one people build their intuition on, and it is the only
one of the 3 that behaves.

## 5. Threats to validity

Two models, one tool, 100 calls, one sitting. This is a comparison, not a
benchmark, and the per cell rates from 5 trials are indicative. The split
between the two groups is not marginal.

The CLI ran at its default decoding settings, which is what practitioners run
but not a controlled temperature.

The failures are synthetic rather than sampled from production. Two shapes of
...
waiting for the MCP handshake, and the model then runs with no tool at all while
looking like it ran normally. A first pass lost 41 of 100 calls that way,
unevenly across models, and was discarded rather than filtered.

## 6. Prior work

The concept that silent tool errors go undetected is published, and this study
is not first to the question.
