...
abstract: >-
  ...
  flat or shrank. On npm the count of accounts able to publish into a tree fell
  faster than the package count: webpack went from 78 packages and 35 publishing
  ...
  against Google's deps.dev on 33 single root stacks, 18 of which matched
  exactly.
date: 2026-08-18
...
under 6,000 registry fetches for this entire study, which is what makes 25
snapshots per stack affordable rather than a sample of convenience.
...
seconds, PyPI took 1,163 requests in 47 seconds, Maven Central took 4,138
requests in 103 seconds. All 3 scans finished with 0 errors.
...
Of the 33 single root stacks compared, 18 matched exactly and 27 landed within
2 packages. The 6 that did not are named in section 5.

Those figures are recomputed here from the shipped `validation.json`. The prose
in the repository's `VALIDATION.md` describes a smaller comparison of 29 stacks
with 14 exact matches, which predates the file that ships beside it. Where the
prose and the data file disagree, the data file is what a reader can check.

## 3. Results

**Resolved month by month with the manifest frozen, 25 of the 39 stacks with a
usable baseline held flat or shrank between 2024 and 2026. 14 grew. On npm the
number of accounts able to push code into a build fell faster than the package
count.**

3 stacks are excluded from that tally because no snapshot in their window
resolved cleanly, which section 5 explains.

| stack | packages then to now | publishing accounts |
|---|---|---|
| `express` + `cors` + `body-parser` | 80 to **71** | 20 to **16** |
| `webpack` | 78 to **68** | 35 to **26** |
| `eslint` | 89 to **86** | 35 to **33** |
| `fastify` | 56 to **49** | 21 to **18** |
| `vue` | 24 to **23** | 11 to **7** |
| `spring-boot-starter-data-jpa` | 65 to **54** | not published |
| `spring-boot-starter-security` | 35 to **28** | not published |
| `okhttp` | 12 to **5** | not published |
| `axios` | 9 to **30** | 7 to 10 |
| `micronaut-http-server-netty` | 42 to **66** | not published |
| `nestjs` | 12 to **21** | 10 to 13 |

The ones that grew are named rather than hidden. `axios` more than tripled its
tree and `micronaut` gained 24 packages.
...
interesting direction. The `express` stack lost 9 packages over the 2 years and
`webpack` lost 10, with no manifest edit on either side. Whatever the registry
is doing, it is not only adding.
...
manifest changing. It changes in both directions. A tree that shrinks by 10
packages is not automatically safer, though a tree that loses 9 publishing
accounts probably is. Neither fact is visible from the file you edit.
...
**6 stacks disagree with `deps.dev` by more than 2 packages**. The
disagreements are not all understood. `log4j-core` resolves to 2 packages here
...
No claim in this paper rests on `netty`. None should until that is understood.
...
signals across 1.63 million npm packages
...
Searched arXiv, Semantic Scholar and Crossref on 29 August 2026. GitHub and
